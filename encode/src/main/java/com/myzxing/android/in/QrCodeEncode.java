 ...
 * 创建人：LorenWang
 * 功能作用：二维码扫描入口，用来进行二维码扫描使用，可以做自定义扫描区域
 ...
 *      所以我就干脆直接在工具类中创建这三个方法，然后将Activity实例传入，用来给一些需要Activity实例的方法调用，然后
 *      Activity的oncreate、onpause、onresume这三个方法中分别调用工具类中的三个方法，用来初始化以及使用二维码扫描
 *      等操作，具体使用不是很复杂，就是实例化工具类，调用相关方法就可以，至于具体的思路直接按调用方法一步一步向下走就
 *      可以了，入口其实就是onresume方法，从这里按方法向下走就可以，每一步都有解释的
 * 修改人：
 * 修改时间：
 * 备注：
 */
